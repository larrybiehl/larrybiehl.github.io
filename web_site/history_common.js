...
var gIs_Firefox   = (navigator.userAgent.indexOf("Firefox") > 0);
...
var ButtonGifs = new Array(6);

ButtonGifs[0]=new Image();
ButtonGifs[0].src = "home.gif";

ButtonGifs[1]=new Image();
ButtonGifs[1].src = "space.gif";

ButtonGifs[2]=new Image();
ButtonGifs[2].src = "first.gif";

ButtonGifs[3]=new Image();
ButtonGifs[3].src = "prev.gif";

ButtonGifs[4]=new Image();
ButtonGifs[4].src = "next.gif";

ButtonGifs[5]=new Image();
ButtonGifs[5].src = "last.gif";

var ButtonDownGifs = new Array(6);

ButtonDownGifs[0]=new Image();
ButtonDownGifs[0].src = "home_down.gif";

ButtonDownGifs[1]=new Image();
ButtonDownGifs[1].src = "space.gif";

ButtonDownGifs[2]=new Image();
ButtonDownGifs[2].src = "first_down.gif";

ButtonDownGifs[3]=new Image();
ButtonDownGifs[3].src = "prev_down.gif";

ButtonDownGifs[4]=new Image();
ButtonDownGifs[4].src = "next_down.gif";

ButtonDownGifs[5]=new Image();
ButtonDownGifs[5].src = "last_down.gif";
...
{    
	...
	//console.log ("in document.ready: CurrentSlide: " + CurrentSlide);
...
function AddListBreakToSlideVector (slideIndex, listBreak)
	...
	if (LastPDFSlide > 0)
		slideVector[LastPDFSlide].NextSlide = slideIndex + 1;
				...
				// Need to update first main list break
		slideVector[0].NextSlide = slideIndex + 2;
				
	
	}	// end "AddListBreakToSlideVector"


function AddMainListBreakToSlideVector (slideIndex, listBreak)
	{
	slideVector[slideIndex] = new AddListBreak (index, listBreak);
	slideVector[slideIndex].listStyle = "B";
	
	if (LastPDFSlide > 0)
		slideVector[LastPDFSlide].NextSlide = slideIndex + 1;
	
	}	// end "AddMainListBreakToSlideVector"
...
function ChangeList()
{
	...
	SetHistoryFileVariables(CurrentSlide);
...
function ClickNext(slide, gifNumber)
{
	...
	document.images[gifNumber+1].src = ButtonGifs[gifNumber].src;
...
{
	//console.log('onkeydown in history_common.js');
...
{       
	...
	pdfMessageElement = document.getElementById("pdfMessage");
			...
			// Have decided not to display the Show List/No List option (10/7/2023)
	//document.getElementById("listButtonBoxID").style.display= 'none';
	...
	if (!gNarrowWidthMode)
		{	
		//console.log ("FinishLoad: gRandomSelectedSlide: " + gRandomSelectedSlide + "  " + CurrentSlide);
		ItemClicked (gRandomSelectedSlide);
		
		}
		
	ImageNumber = gRandomSelectedSlide;
...
// This function handles the go home button; go to the introduction page
function GoHome() 
{
	window.location.href = "../web_site/histories_memories_home_page.html";
...
{  
	...
	SetHistoryFileVariables(iIndex);
	...
	if (gNarrowWidthMode)
		...
		//console.log ("ItemClicked: pdfElement.src: " + pdfElement.src);
		
		}	// end "else !gNarrowWidthMode"

			// Handle making the title in the list bold for the title being shown.
	if (LastSlideShown >= 0)
		{
	...
	*/	
	...
	//console.log ("ItemClicked: LastSlideShown: " + LastSlideShown);
...
<P>&nbsp&nbsp&nbsp<a href="Deal/Deal_History_Scrapbook.pdf">Deal History-Scrapbook</a></P>
<P>&nbsp&nbsp&nbsp<a href="Deal/Lulu_Denney_Deal_History_Book.pdf">Lulu Denney Deal History Book</a></P>
<P>&nbsp&nbsp&nbsp<a href="Deal/Albert_Deal_Autograph_Book.pdf">Albert Deal Autograph Book</a></P>
<P>&nbsp&nbsp&nbsp<a href="Deal_Stouffer_Pictures_1910-1920.pdf">Deal & Stouffer Pictures: 1910-1920</a></P>
</details>
*/
	...
	if (ListLoadedCode == 0)
		...
		for (index=0; index<=LastSlide; index++)
			...
			if (slideVector[index].listBreak != '')
				{
				//console.log ("slideVector[index].listBreak: " + slideVector[index].listBreak)
				if (gDetailsFlag == 1)
					{
							// Need to add closing details
							
					//listElement.innerHTML = listElement.innerHTML + '</details>';
					content += '</details>';
					gDetailsFlag = 0;
					
					//console.log ("content1: " + content)
							
					}	// end "if (detailsFlag == 1)"
				
				if (slideVector[index].listStyle == 'B')
					content = content + '<p style="color:green;"><b>' + slideVector[index].listBreak + '</b></p>';
	
				else // slideVector[index].listStyle != 'B'
					{
					//listElement.innerHTML = listElement.innerHTML + '<details><summary>' + slideVector[index].listBreak + '</summary>';
					content += '<details><summary>' + slideVector[index].listBreak + '</summary>';
					gDetailsFlag = 1;
				...
				}	// end slideVector[index].listBreak != ''
				...
				content += '<p id="d' + index + '"><a href="JavaScript:window.top.ItemClicked(' + index + ')">' + slideVector[index].listTitle + '</a></p>';
				}
...
function MouseDown(gifNumber)
{
	...
	console.log ("MouseDown: ButtonDownGifs[0].src: " + ButtonDownGifs[0].src);
	console.log ("MouseDown: ButtonDownGifs[1].src: " + ButtonDownGifs[1].src);
	console.log ("MouseDown: ButtonDownGifs[2].src: " + ButtonDownGifs[2].src);
	console.log ("MouseDown: ButtonDownGifs[3].src: " + ButtonDownGifs[3].src);
	console.log ("MouseDown: ButtonDownGifs[4].src: " + ButtonDownGifs[4].src);
	console.log ("MouseDown: ButtonDownGifs[5].src: " + ButtonDownGifs[5].src);
	
	console.log ("MouseDown: ButtonGifs[0].src: " + ButtonGifs[0].src);
	console.log ("MouseDown: ButtonGifs[1].src: " + ButtonGifs[1].src);
	console.log ("MouseDown: ButtonGifs[2].src: " + ButtonGifs[2].src);
	console.log ("MouseDown: ButtonGifs[3].src: " + ButtonGifs[3].src);
	console.log ("MouseDown: ButtonGifs[4].src: " + ButtonGifs[4].src);
	console.log ("MouseDown: ButtonGifs[5].src: " + ButtonGifs[5].src);
	*/
	doDownGifFlag = 0;
	switch (gifNumber)
		{
	...
	if (doDownGifFlag == 1)
		document.images[gifNumber+1].src = ButtonDownGifs[gifNumber].src;
...
//	This function handles setting the global variables to be used to create the pdf frame.

function SetHistoryFileVariables (iIndex)
{  
	lSlideVector = slideVector[iIndex];
	//console.log ("SetHistoryFileVariables: iIndex " + iIndex);
	//console.log ("SetHistoryFileVariables: path " + slideVector[iIndex].path);
	//console.log ("SetHistoryFileVariables: fileName " + slideVector[iIndex].fileName);
	...
	//console.log ("SetHistoryFileVariables: ImageName " + ImageName);
...
function WindowSizeChanged()
{
	...
	if (isMobileDevice)
		...
		if (gNarrowWidthMode)
			...
			$('#listView').attr("data-intro", "Select name in the list to show history information in a separate window. Use browser left arrow to go back to this list.");
			...
			$('#listView').attr("data-intro", "Select name in the list to show history information for that family to the right.");
	...
	if (!isMobileDevice)
		...
		if (window.innerWidth > gWideScreenThreshhold)
			{
			gListWidth = 230;
			$('.listButtonBox').css({"width": "225px"});	
			//$('.listButtonBox').css({"padding-left": "40px"});	
			//width = gListWidth + 'px';
